    ...
    steps:
      ...
      - name: Setup PHP
        ...
        with:
          php-version: "8.3"
      - name: Run PHP Lint
        run: bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/php-lint/master/src/pipeline.sh)
```
...
```bash
bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/php-lint/master/src/pipeline.sh)
```
